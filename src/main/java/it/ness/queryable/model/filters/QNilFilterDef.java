...
import static it.ness.queryable.builder.Constants.NIL_PREFIX;
import static it.ness.queryable.builder.Constants.QNILL_ANNOTATION_NAME;
...
public class QNilFilterDef extends FilterDefBase {
    ...
    public QNilFilterDef parseQFilterDef(String entityName, FieldSource<JavaClassSource> f, boolean qClassLevelAnnotation) {
        AnnotationSource<JavaClassSource> a = f.getAnnotation(QNILL_ANNOTATION_NAME);
        if (null == a) {
            return null;
        }
        String prefix = getQAnnotationValue(a, "prefix", NIL_PREFIX);
        ...
        if (!supportedTypes.contains(fieldType)) {
            log.error(String.format("%s is not applicable for fieldType: %s fieldName: %s", QNILL_ANNOTATION_NAME, fieldType, name));
            return null;
        }
